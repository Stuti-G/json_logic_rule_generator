...
class GenerateRuleRequest(BaseModel):
    ...
    context_docs: Optional[List[str]] = Field(
        default=None,
        description="Optional additional policy documents for context"
    )


class KeyMapping(BaseModel):
    user_phrase: str = Field(..., description="The phrase extracted from user's prompt")
    mapped_to: str = Field(..., description="The SAMPLE_STORE_KEY value we mapped it to")
...
class GenerateRuleResponse(BaseModel):
    ...
    key_mappings: List[KeyMapping] = Field(..., description="How user phrases mapped to keys")
    ...
    rag_context_used: Optional[List[str]] = Field(
        default=None,
        description="Policy document IDs that were used for context"
    )
...
class ErrorResponse(BaseModel):
    ...
    unmapped_phrases: Optional[List[str]] = Field(
        default=None,
        description="Phrases we couldn't map to any key"
    )
