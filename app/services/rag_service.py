...
class RAGService:
    ...
    def initialize_policy_embeddings(self):
        ...
        logger.info(f"Created {len(self.chunks)} chunks from {len(self.policy_documents)} documents")
        
        self.policy_embeddings = self.embedding_service.embed_texts(self.chunks)
        
        logger.info("Policy embeddings computed successfully")
    
    def retrieve_relevant_policies(self, query: str, top_k: int = 3, threshold: float = 0.2) -> List[str]:
